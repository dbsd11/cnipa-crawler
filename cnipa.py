...
patents = read_json(records_path, 'utf8', {}) # request_id - content
if patents == None:
    patents = {}
    save_json(records_path, patents, 'utf8')
reader = easyocr.Reader(['en'], model_storage_directory='./data_tmp/nn_model/EasyOCR', download_enabled=True)
...
def download_item(b, try_times: int = 10):
    ...
    web.send_browser_key(b, Keys.CONTROL + 'a', '//*[@id="app"]/div[1]/section/div/div/div[7]/div/div/div[2]/div[3]/div/input', 'xpath')
...
web.random_sleep(3)
choice_box = pyautogui.locateOnScreen('./data_tmp/imgs/chrome___settings_content_popups.png', confidence=0.9)
...
while now_page_idx <= sum_page:
    # 切换至结果窗口并获取该页的条目数
    switch_window('result', b)
    sum_items = len(etree.HTML(b.page_source).xpath("//li[@class='text2 color']/div/span"))
